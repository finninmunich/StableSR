            ...
            with model.ema_scope():
                ...
                for n in trange(len(init_image_list), desc="Sampling"):
                    ...
                    init_latent = model.get_first_stage_encoding(
                        model.encode_first_stage(init_template))  # move to latent space
                    ...
                    for i in range(1):
                        ...
                        Image.fromarray(x_sample.astype(np.uint8)).resize((3840,2160)).save(
                            os.path.join(outpath, basename + '.jpg'))
                        # init_image = torch.clamp((init_image + 1.0) / 2.0, min=0.0, max=1.0)
                        # init_image = 255. * rearrange(init_image[i].cpu().numpy(), 'c h w -> h w c')
                        # Image.fromarray(init_image.astype(np.uint8)).resize((3840, 2160)).save(
                        #     os.path.join(outpath, basename + '_lq.jpg'))
                    # clean cuda memory
                    del init_template
                    torch.cuda.empty_cache()
                    del init_latent
